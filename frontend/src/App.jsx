...
const App = () => {
  ...
  const openai = new OpenAI({
    ...
    defaultHeaders: {
      "HTTP-Referer": import.meta.env.VITE_SITE_URL || window.location.origin, // Use env var or fallback
      "X-Title": import.meta.env.VITE_SITE_NAME || "Fertilizer Recommendation System",
    },
  ...
  const handleGenAi = async (fertilizer) => {
    ...
    try {
      ...
      <Recommended Combinations of Fertilizers for optimal plant growth>
      ...
      setGenAiResult(completion.choices[0].message.content);
    } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Call handleGenAi with the new result after state is updated
      await handleGenAi(recommendedFertilizer);
    } catch (err) {
    ...
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-green-100 flex flex-col items-center p-6">
      <div className="w-full max-w-2xl">
        <header className="text-center mb-8">
          <h1 className="text-4xl font-bold text-green-800">Fertilizer Recommendation🌱</h1>
          <p className="text-gray-600 mt-2">Find the perfect fertilizer for your plants based on soil and environmental conditions</p>
        </header>
        ...
        {result && !error && (
          <div className="mt-6 bg-green-50 border border-green-200 p-6 rounded-lg shadow-md w-full">
            ...
            <p className="mt-4 text-sm text-gray-600 text-center">Based on your input parameters, this is the optimal fertilizer for your plants.</p>
          </div>
        )}
      </div>

      <footer className="mt-12 text-center text-gray-600 text-sm">
        <p>© {new Date().getFullYear()} Fertilizer Recommendation System</p>
      </footer>
    </div>
  );
};
